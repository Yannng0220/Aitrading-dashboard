...
export default function Dashboard() {
  ...
  const [priceStreamStatus, setPriceStreamStatus] = useState<'connecting' | 'live' | 'fallback'>('connecting');
  ...
  // Realtime price stream via SSE (server pushes prices; if it fails, fallback to client polling)
  useEffect(() => {
    ...
    const connect = () => {
      if (stopped) return;
      setPriceStreamStatus('connecting');
      try {
      ...
      es.addEventListener('prices', (evt: MessageEvent) => {
        try {
          ...
          const incoming = parsed?.prices as Record<string, number> | undefined;
          const ts = Number(parsed?.ts ?? Date.now());
          if (!incoming || typeof incoming !== 'object') return;
          lastPriceTsRef.current = ts;
          pricesRef.current = incoming;
          setPrices(incoming);

          // Maintain history for strategies/mini charts
          Object.keys(incoming).forEach(s => {
            if (!historyMapRef.current[s]) historyMapRef.current[s] = [];
            const currentHistory = historyMapRef.current[s];
            historyMapRef.current[s] = [...currentHistory.slice(-19), incoming[s]];
          });

          setPriceStreamStatus('live');
        } catch {
          // ignore malformed
        }
      });
      ...
      es.addEventListener('error', () => {
        // SSE will auto-retry, but if server is down we drop to fallback polling
        setPriceStreamStatus(prev => (prev === 'live' ? 'fallback' : prev));
      });
    ...
    // If stream stays dead for a bit, attempt reconnect (and allow fallback polling meanwhile)
    reconnectTimer = window.setInterval(() => {
      if (stopped) return;
      const age = Date.now() - (lastPriceTsRef.current || 0);
      if (age > 5000) {
        setPriceStreamStatus('fallback');
        try {
          es?.close();
  ...
  // Fallback polling (only used when SSE is not live)
  useEffect(() => {
    if (priceStreamStatus !== 'fallback') return;
    let cancelled = false;
    const poll = async () => {
      const allPrices = await fetchAllBybitTickers();
      if (cancelled) return;
  ...
  // Simulation Loop (agent updates) - uses latest pricesRef (from SSE or fallback poll)
  useEffect(() => {
    if (isPaused || !isStarted) return;

    const interval = setInterval(() => {
      const allPrices = pricesRef.current;
      ...
      <header className="border-b border-white/5 bg-black/40 backdrop-blur-md sticky top-0 z-50">
        <div className="max-w-[1600px] mx-auto px-6 h-16 flex items-center justify-between">
          ...
          <div className="flex items-center gap-6">
            <div className={cn(
              "flex items-center gap-2 px-3 py-1.5 border rounded-full text-[10px] font-bold tracking-widest uppercase",
              priceStreamStatus === 'live' && "bg-emerald-500/10 border-emerald-500/20 text-emerald-500",
              priceStreamStatus === 'connecting' && "bg-white/5 border-white/10 text-white/50",
              priceStreamStatus === 'fallback' && "bg-amber-500/10 border-amber-500/20 text-amber-400",
            )}>
              <span className={cn(
                "w-1.5 h-1.5 rounded-full",
                priceStreamStatus === 'live' && "bg-emerald-500 animate-pulse",
                priceStreamStatus === 'connecting' && "bg-white/30 animate-pulse",
                priceStreamStatus === 'fallback' && "bg-amber-400 animate-pulse",
              )} />
              {priceStreamStatus === 'live' ? 'SSE Live' : priceStreamStatus === 'connecting' ? 'Connecting' : 'Fallback Polling'}
            </div>
            ...
            <button 
              ...
              className="p-2 hover:bg-rose-500/10 rounded-full transition-colors group"
              title="重置所有代理人"
            >
              <RotateCcw className="w-5 h-5 text-rose-500/60 group-hover:text-rose-500" />
            </button>
                            ...
                            <div>
                              <div className="flex items-center gap-2">
                                ...
                                <span className={cn(
                                  ...
                                  {(pos.side || 'LONG') === 'SHORT' ? 'SHORT' : 'LONG'}
                                </span>
                            ...
                            <div className="flex items-center justify-between mb-2">
                              <div className="flex items-center gap-2">
                                <span className={cn(
                                  ...
                                  {trade.positionSide && ` • ${trade.positionSide}`}
